...
export const screeningCardApiSlice = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    screenCandidates: builder.mutation<BatchScreeningResponse, ScreenCandidatesPayload>({
      query: (data) => ({
        url: "/api/v1/employerMatching/screen",
        method: "POST",
        body: data,
      }),
    }),
    ...
    getScreeningProgress: builder.query<ScreeningProgressResponse, string>({
      query: (batch_id) => ({
        url: `/api/v1/employerMatching/progress/${batch_id}`,
        method: "GET",
      }),
    }),
    ...
    getScreeningResults: builder.query<ScreeningCardsListResponse, GetScreeningResultsParams>({
      query: (params) => ({
        url: `/api/v1/employerMatching/results`,
        method: "GET",
        params,
      }),
    ...
    >({
      query: ({ job_id, candidate_id }) => ({
        url: `/api/v1/employerMatching/details/${job_id}/${candidate_id}`,
        method: "GET",
      }),
    }),
    ...
    >({
      query: ({ id, invite_status }) => ({
        url: `/api/v1/employerMatching/${id}/invite`,
        method: "PATCH",
        body: { invite_status },
      }),
    }),
