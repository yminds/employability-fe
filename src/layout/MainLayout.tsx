...
const MainLayout: React.FC<MainLayoutProps> = ({ children }) => {
  ...
  if (currentPath === "/auth/github/callback" || currentPath === "/auth/linkedin/callback") {
  ...
  useEffect(() => {
    if (user && !experience_level) {
      navigate(
        jobApplication ? `/setexperience?job_application=${encodeURIComponent(jobApplication)}` : "/setexperience"
      );
    }
  }, [user, experience_level]);

  const finalNoSidebarRoutes = useMemo(() => {
    if (!user) {
      return noSidebarRoutes.includes("/") ? noSidebarRoutes : [...noSidebarRoutes, "/"];
    } else {
      return noSidebarRoutes.includes("/") ? noSidebarRoutes.filter((route) => route !== "/") : noSidebarRoutes;
    }
  }, [user, noSidebarRoutes]);
  ...
  const shouldDisplaySidebar = (): boolean => {
    ...
    const isNoHeaderRoute = finalNoSidebarRoutes.some((route) => {
