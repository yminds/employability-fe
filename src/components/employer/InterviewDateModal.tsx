...
export function InterviewDateModal({ isOpen, onClose, onConfirm, deadlineDate }: InterviewDateModalProps) {
  ...
  const isPrevMonthDisabled = () => {
    ...
    return (
      currentMonth.getFullYear() === initialMonth.getFullYear() && currentMonth.getMonth() === initialMonth.getMonth()
    )
  }
  ...
  const isNextMonthDisabled = () => {
    ...
    if (nextMonth.getFullYear() === deadlineDate.getFullYear() && nextMonth.getMonth() > deadlineDate.getMonth()) {
      return true
    }
  ...
  const renderCalendar = () => {
    ...
    const emptyCells = Array.from({ length: firstDay }, (_, i) => <div key={`empty-${i}`} className="h-10 w-10"></div>)
    ...
    const dayCells = Array.from({ length: daysInMonth }, (_, index) => {
      ...
      return (
        <button
          ...
          disabled={isDisabled}
          title={isDisabled ? (isPastDate ? "Past date" : "Beyond deadline") : ""}
        >
          {day}
        </button>
      )
      ...
      <DialogContent className="sm:max-w-md p-0 gap-0 rounded-lg">
        <DialogHeader className="p-6 pb-0">
          <div className="flex items-center justify-between">
            <DialogTitle className="text-h2 text-[#001630]">When can you complete the interview?</DialogTitle>
          </div>
        ...
        <div className="p-6">
          <div className="border border-[#eceef0] rounded-lg p-4">
            ...
            <div className="grid grid-cols-7 gap-1 mb-2">
              {daysOfWeek.map((day) => (
                <div key={day} className="h-10 w-10 flex items-center justify-center text-[#68696b] text-sm">
                  {day}
                </div>
              ))}
            </div>
          ...
          <div className="flex justify-between mt-6">
            <Button variant="outline" onClick={onClose} className="px-8 border-[#d6d7d9] text-[#202326]">
              Cancel
            </Button>
            <Button
              onClick={handleConfirm}
              disabled={!selectedDate}
              className="px-8 bg-[#001630] hover:bg-[#001630]/90 text-white"
            >
              Confirm Date
            </Button>
          </div>
