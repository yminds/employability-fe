import React from "react";
import { Eye, MapPin, Briefcase, Building, Clock } from "lucide-react";
import { Button } from "@/components/ui/button";

interface Skill {
  _id: string;
  name: string;
  icon?: string;
}

interface ScreeningQuestion {
  question: string;
  type: "multiple_choice" | "yes_no" | "text" | "numeric";
  options?: string[];
  is_mandatory: boolean;
  is_eliminatory: boolean;
  ideal_answer?: string;
  customField?: string;
  customFieldValue?: string;
}

interface JobPreviewPanelProps {
  jobTitle: string;
  companyName: string;
  location: string;
  jobType: string;
  workplaceType: string;
  description?: string;
  skills?: Skill[];
  screeningQuestions?: ScreeningQuestion[];
  expanded?: boolean;
  onPreviewClick?: () => void;
}

const JobPreviewPanel: React.FC<JobPreviewPanelProps> = ({
  jobTitle = "",
  companyName = "",
  location = "",
  jobType = "",
  workplaceType = "",
  description = "",
  skills = [],
  screeningQuestions = [],
  expanded = false,
  onPreviewClick,
}) => {
  // Format job type for display
  const formatJobType = (type: string): string => {
    return type
      .split("-")
      .map(word => word.charAt(0).toUpperCase() + word.slice(1))
      .join(" ");
  };

  // Get formatted question text
  const getFormattedQuestion = (question: ScreeningQuestion) => {
    if (!question.customField || !question.customFieldValue) return question.question;
    return question.question.replace(`[${question.customField}]`, question.customFieldValue);
  };

  return (
    <div className="w-full bg-white rounded-xl shadow-sm">
      {/* Basic Preview (Always Visible) */}
      <div className="p-6 flex justify-between items-start">
        <div className="flex gap-5">
          {/* Company logo/icon */}
          <div className="w-[70px] h-[70px] relative bg-[#ecedef] rounded-full border-white/0 overflow-hidden">
            <div className="w-[96.92px] h-[102.85px] left-[-12.38px] top-[-8.62px] absolute">
              <div className="w-[96.92px] h-[102.85px] left-0 top-0 absolute" />
              <div className="w-[43.61px] h-[56.43px] left-[26.38px] top-[24.07px] absolute">
                <div className="w-[30.13px] h-[56.43px] left-0 top-0 absolute bg-[#cdead9]" />
                <div className="w-[30.13px] h-[30.23px] left-[13.49px] top-[26.20px] absolute bg-[#bbddc9]" />
                <div className="w-[24.28px] h-[17.13px] left-[3.15px] top-[3.53px] absolute inline-flex flex-col justify-start items-start gap-[3.23px]">
                  <div className="self-stretch h-[2.15px] bg-[#a6c4b2]" />
                  <div className="self-stretch h-[2.15px] bg-[#a6c4b2]" />
                  <div className="self-stretch h-[2.15px] bg-[#a6c4b2]" />
                  <div className="self-stretch h-[2.15px] bg-[#a6c4b2]" />
                </div>
              </div>
            </div>
          </div>
          
          {/* Job details */}
          <div className="inline-flex flex-col justify-center items-start gap-1">
            <div className="flex flex-col justify-start items-start gap-4">
              <div className="flex flex-col justify-start items-start gap-1">
                <div className="justify-center text-[#414347] text-xl font-medium font-['Ubuntu'] leading-loose">
                  {jobTitle}
                </div>
                <div className="flex flex-col justify-center items-start gap-1">
                  <div className="justify-end text-[#414347] text-base font-normal font-['DM_Sans'] leading-relaxed tracking-tight">
                    {companyName}
                  </div>
                </div>
              </div>
              <div className="inline-flex justify-start items-center gap-2">
                <div className="w-[13px] h-4 bg-[#a6c4b3]" />
                <div className="justify-end text-[#8f9091] text-base font-normal font-['DM_Sans'] leading-normal tracking-tight">
                  {location}
                </div>
              </div>
            </div>
          </div>
        </div>

        {/* Preview button */}
        <div 
          className="rounded flex justify-center items-center gap-2 cursor-pointer"
          onClick={onPreviewClick}
        >
          {/* <div className="w-[20.39px] h-3.5 bg-[#10b753]" /> */}
          <div className="justify-end text-[#10b753] text-sm font-medium font-['SF_Pro_Display'] leading-tight tracking-tight">
            Preview
          </div>
        </div>
      </div>

      {/* Extended Preview (Only visible when expanded=true) */}
      {expanded && (
        <div className="px-6 pb-6 border-t border-gray-100 pt-4">
          <div className="space-y-4">
            {/* Job metadata */}
            <div className="flex flex-wrap gap-4 text-sm text-gray-600">
              <div className="flex items-center gap-1">
                <Briefcase className="w-4 h-4 text-gray-400" />
                <span>{formatJobType(jobType)}</span>
              </div>
              <div className="flex items-center gap-1">
                <Building className="w-4 h-4 text-gray-400" />
                <span>{formatJobType(workplaceType)}</span>
              </div>
            </div>
            
            {/* Description */}
            {description && (
              <div>
                <h3 className="font-medium text-gray-800 mb-1">Description</h3>
                <p className="text-sm text-gray-600 line-clamp-3">{description}</p>
                {description.length > 180 && (
                  <button className="text-sm text-blue-600 mt-1">See more</button>
                )}
              </div>
            )}
            
            {/* Skills */}
            {skills.length > 0 && (
              <div>
                <h3 className="font-medium text-gray-800 mb-2">Required Skills</h3>
                <div className="flex flex-wrap gap-2">
                  {skills.map((skill) => (
                    <div
                      key={skill._id}
                      className="bg-blue-50 text-blue-800 text-xs px-2 py-1 rounded"
                    >
                      {skill.name}
                    </div>
                  ))}
                </div>
              </div>
            )}
            
            {/* Screening Questions */}
            {screeningQuestions.length > 0 && (
              <div>
                <h3 className="font-medium text-gray-800 mb-2">Screening Questions</h3>
                <div className="space-y-2">
                  {screeningQuestions.slice(0, 3).map((question, index) => (
                    <div key={index} className="text-sm">
                      <p className="text-gray-700">{index + 1}. {getFormattedQuestion(question)}</p>
                      {question.is_eliminatory && (
                        <span className="text-xs text-red-600 ml-4">Must-have qualification</span>
                      )}
                    </div>
                  ))}
                  {screeningQuestions.length > 3 && (
                    <div className="text-sm text-gray-500">
                      +{screeningQuestions.length - 3} more questions
                    </div>
                  )}
                </div>
              </div>
            )}
          </div>
        </div>
      )}
    </div>
  );
};

export default JobPreviewPanel;