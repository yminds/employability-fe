...
interface InterviewModalProps {
  isOpen: boolean;
  onClose: (successMessage?: string) => void; // Modified to accept success message
...
export default function InterviewModal({
  ...
  const handleWorkerMessage = useCallback(
      ...
      switch (type) {
        ...
        case "BATCH_CREATED":
          ...
          const candidateCount =
            data.candidateCount || selectedCandidates.length;
          toast.info(
            `Sending ${candidateCount} interview invitation${
              candidateCount !== 1 ? "s" : ""
            }...`
          );
          break;
        ...
        case "INVITATION_COMPLETE":
          ...
          if (total > 0 && completed > 0) {
            ...
            // Start animation to close the modal
            setIsVisible(false);

            // Wait for animation to complete, then close with the success message
            setTimeout(() => {
              onClose(message); // Pass success message to parent component

              if (onSuccess && total > 0 && completed > 0) {
                onSuccess();
              }
            }, 300);
          }
    ...
    [selectedCandidatesCount, selectedCandidates, onClose, onSuccess]
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-end">
      ...
      <div
        className={`absolute top-2 bottom-2 right-2 bg-white w-full max-w-6xl shadow-lg rounded-lg transform transition-transform duration-300 ease-in-out flex flex-col ${
        ...
        <div className="flex-grow overflow-y-auto">
          <div className="p-6">
            <div className="flex flex-row gap-10">
              {/* Left column - Screening interview section */}
                        ...
                        .map((candidate, index) => (
                          ...
                          >
                            {candidate?.profile_image ? (
                              <img
                                src={
                                  candidate.profile_image || "/placeholder.svg"
                                }
                        ...
                        >
                          ...
                          <div className="flex transition-transform duration-200 hover:scale-105">
                            <img
                              ...
                              alt="Full Interview"
                              className="object-contain h-[190px] max-h-full"
                              style={{ maxWidth: "none" }}
                            />
                          </div>
                        </div>
              ...
              {/* Right column - Subsequent content */}
              <div className="w-1/2 space-y-6">
                {!batchId && (
                  <>
