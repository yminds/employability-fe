...
const CameraCheck: React.FC<CameraCheckProps> = ({ onCameraChange, onScaleChange }) => {
  const { cameras, selectedCamera, zoom, videoRef, handleCameraChange, handleZoomChange } =
    useCameraCheck();
  ...
  return (
    <div
      className={`bg-[#FAFAFA] p-6 pe-8 flex flex-col gap-4 rounded-xl ${
        isCameraSelected ? "border-[#10B754] border-2" : "border border-[#DBDBDB]"
      }`}
    >
      <div className="flex justify-between items-center">
        <div className="text-[#333] text-xl not-italic font-medium leading-[normal] flex items-center gap-5">
          <span className="flex w-10 h-10 p-2 justify-center items-center gap-1 bg-white border border-[#ddd] rounded-[42px]">
            <img
              className="w-6 h-4"
              src="./src/assets/images/screen-setup/camera.svg"
              alt="Camera Icon"
            />
          </span>
        ...
        {isCameraSelected && (
          <div className="flex items-center gap-2 relative">
            ...
            <img
              ...
              src="./src/assets/images/screen-setup/check_circle.svg"
              alt="Checked"
            />
          </div>
        )}
      ...
      <div className="w-full flex h-[374px] p-0 items-center justify-center gap-5 border-2 border-[#10B754] rounded-xl relative">
        {selectedCamera ? (
          <>
            <img
              className="absolute z-10 w-[100px] h-[150px] left-1/2 transform -translate-x-1/2 top-20"
              src="./src/assets/images/screen-setup/head-skeleton.svg"
              alt="Head Skeleton"
            />
            <img
              className="absolute z-10 w-[375px] h-[100px] bottom-0"
              src="./src/assets/images/screen-setup/body-skeleton.svg"
              alt="Body Skeleton"
            />
            <div
      ...
      <div className="flex flex-col gap-2 mt-4">
        <div className="flex justify-between">
          <span className="text-[#4C4C4C] text-sm font-medium">Camera Zoom</span>
          <span className="text-[#4C4C4C] text-sm font-medium">{zoom - 100}%</span>
        </div>
        <input
          type="range"
          min="100"
          ...
          style={{
            background: `linear-gradient(to right, #4CAF50 ${(zoom - 100) / 2}%, #ddd ${
              (zoom - 100) / 2
            }%)`,
          }}
        />
