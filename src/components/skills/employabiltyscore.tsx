...
const EmployabilityScore: React.FC<EmployabilityScoreProps> = ({
  goalId,
  goalName,
  isSkillsUpdated,
}) => {
  ...
  const [getUserSkills, { data: skillsData }] = useGetUserSkillsMutation(); // isLoading, isError, error
  ...
  const averageVerifiedRating =
    skillsData && skillsData?.data?.mandatory.length > 0
      ? totalVerifiedRating / skillsData?.data?.mandatory.length
      : 0;
console.log("working", ProfileAvatar);
    ...
    <div className="bg-white flex flex-col w-[100%] rounded-lg  p-[30px] gap-6 md:mt-0 sm:mt-0">
      <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col items-start">
          ...
          <p className="text-[#909091]0 text-[14px] font-medium leading-[24px] tracking-[-0.2px]">
            {goalName}
          </p>
        </div>
      </div>
      ...
      <div className="p-4 w-[100%] h-[92px] bg-green-50 rounded-lg flex items-center space-x-4">
        <div className="relative w-[60px] h-[60px] flex items-center justify-center border rounded-full">
          ...
          <CircularProgress
            progress={averageVerifiedRating * 10}
            size={60}
            strokeWidth={6}
            showText={false}
          />
        ...
        <div>
          <p className="text-2xl font-bold text-gray-900">
            {averageVerifiedRating > 0
              ? averageVerifiedRating.toFixed(2)
