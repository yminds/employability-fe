...
interface UploadLinksStepProps {
  ...
  setPersistedFiles: (persistedFiles: { [key: string]: { url: string; key: string } }) => void
...
const UploadLinksStep: React.FC<UploadLinksStepProps> = ({
    ...
    if (persistedFiles.images?.url) {
      try {
        ...
        const existingImages = parsedImages.map((url: string, index: number) => ({
          ...
          progress: 100,
          file: new File([], `Project Image ${index + 1}`),
          s3Key: url,
        }))
  ...
  useEffect(() => {
    ...
    if (persistedFiles.images?.url) {
      try {
        ...
        console.log("img in useeffect", images)
  ...
  useEffect(() => {
    if (existingImages) {
      ...
      const existingImageUrls = urls.map((url, index) => ({
        id: `existing-${index}`,
        url,
        status: "uploaded" as const,
        progress: 100,
      ...
      setImageUploadState({
        files: existingImageUrls.reduce(
          (acc, img) => ({
            ...acc,
            [img.id]: img,
          }),
  ...
  const handleImagesUpload = async (fileList: FileList | null) => {
    ...
    files.forEach((file) => {
      ...
      newFiles[id] = {
        file: new File([file], `Project Image ${Object.keys(imageUploadState.files).length + 1}`, {
          type: file.type,
        }),
        id,
        status: "uploading",
  ...
  const handleThumbnailDelete = async () => {
    ...
    try {
      ...
      if (key) {
        // Delete from S3
        try {
          await axios.delete("http://localhost:3000/api/v1/s3/delete", { data: { key } })
        } catch (error) {
  ...
  const handleImageRemove = async (id: string) => {
    ...
    if (imageFile && imageFile.s3Key) {
      try {
        await axios.delete("http://localhost:3000/api/v1/s3/delete", { data: { key: imageFile.s3Key } })
        setImageUploadState((prev) => {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-gray-700">{file ? file.name : "Uploaded Thumbnail"}</p>
            </div>
          </div>
      ...
      <div className="w-full flex flex-col p-3 rounded-lg bg-[#F0F5F3]">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-gray-700">{file ? file.name : "Uploaded Synopsis"}</p>
            </div>
          </div>
        ...
        {status === "uploading" && <UploadProgressBar progress={progress || 0} />}
        {synopsisUrl && (
          <a href={synopsisUrl} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline mt-2">
            View Document
          </a>
        )}
        {status === "error" && <p className="text-red-500 text-sm mt-1">Upload failed</p>}
