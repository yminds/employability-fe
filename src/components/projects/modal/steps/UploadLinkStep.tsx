...
interface UploadLinksStepProps {
  ...
  setPersistedFiles: (
    files: ((prevFiles: PersistentFiles) => PersistentFiles) | PersistentFiles
  ) => void;
...
const UploadLinksStep: React.FC<UploadLinksStepProps> = ({
  ...
  const [synopsisUploadState, setSynopsisUploadState] = useState<
    FileUploadState<File>
  >({
    file: null,
    ...
    originalFileName: persistedFiles.synopsisDoc?.url ? extractFileNameFromUrl(persistedFiles.synopsisDoc.url) : undefined
  });
  ...
  const [imageUploadState, setImageUploadState] = useState<{
    ...
    if (persistedFiles.images?.url) {
      try {
        ...
        try {
          parsedImages = typeof persistedFiles.images.url === "string"
        ...
        return {
          files: existingImages.reduce((acc, img) => ({
            ...acc,
            [img.id]: img
          }), {})
        };
    ...
    // If no persistedFiles, check projectDetails
      ...
      const images = Array.isArray(projectDetails.images)
  ...
  useEffect(() => {
    if (projectDetails?.images) {
      ...
      setImageUploadState({
        files: existingImageFiles.reduce((acc, img) => ({
          ...acc,
          [img.id]: img
        }), {})
      });
  ...
  const handleImagesUpload = async (fileList: FileList | null) => {
    ...
    // Upload new files
    const uploadPromises = Object.entries(newFiles).map(
      async ([id, imageFile]) => {
        try {
          const formData = new FormData();
  ...
  const ThumbnailPreview = () => {
    ...
    const displayName = originalName.length > 15 
      ? `${originalName.slice(0, 15)}...${originalName.slice(-3)}` 
      : originalName;
    ...
    return (
      <div 
        className="w-full flex flex-col p-3 rounded-lg bg-[#F0F5F3]"
        title={originalName}
      >
  ...
  const ImageFileDisplay = ({ id }: { id: string }) => {
    ...
    const imageUrl =
      imageFile.url || (imageFile.file && URL.createObjectURL(imageFile.file));
  
    // Trim the displayed name to a maximum of 15 characters
    const displayName = imageFile.file.name.length > 15 
    ...
    return (
      ...
      >
        ...
        {imageFile.status === "uploading" && (
          <UploadProgressBar progress={imageFile.progress || 0} />
        )}
      </div>
    );
  };
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 ">
              ...
              <div className="flex gap-2 relative">
                <div className="relative flex-1">
                  ...
                  <Input
                    ...
                    value={link}
                    onChange={(e) =>
                      handleGithubLinkChange(index, e.target.value)
                    }
      ...
      <section className="space-y-4">
        ...
        <div className="flex items-start gap-5 w-full">
          <div className="w-1/2 h-[320px] flex flex-col justify-center itemscenter gap-4 rounded-[13px] border border-dashed border-[#909091]">
            <input
              type="file"
              id="synopsisPdfInput"
