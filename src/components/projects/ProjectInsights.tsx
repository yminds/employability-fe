...
const ProjectInsights: React.FC<ProjectInsightsProps> = ({
  ...
  const goalData =
    Array.isArray(goalDetails) && goalDetails.length > 0
      ? goalDetails[0]
      : null;

  const [getUserSkills, { data: skillsData }] = useGetUserSkillsMutation();
  const { data: projectDetails, isLoading: projectsLoading } =
  ...
  useEffect(() => {
    if (projectDetails?.data) {
      const verifiedProjects = projectDetails.data.filter(
        (project) => project.status === "Verified"
      );

      if (verifiedProjects.length > 0) {
        const totalScore = verifiedProjects.reduce(
  ...
  useEffect(() => {
    if (skillsData?.data?.mandatory && projectDetails?.data) {
      ...
      const skillCounts: SkillCounts = mandatorySkills.reduce(
        (acc: SkillCounts, skill) => {
          acc[skill] = 0;
          return acc;
        },
        {}
  ...
  useEffect(() => {
    const updateSkillsContainerHeight = () => {
      if (skillsContainerRef.current) {
        const containerTop =
          skillsContainerRef.current.getBoundingClientRect().top;
  ...
  useEffect(() => {
    if (skillsContainerRef.current) {
      setShowScrollbar(
        skillsContainerRef.current.scrollHeight >
          skillsContainerRef.current.clientHeight
      );
    }
  }, [skillsContainerRef]);
  ...
  return (
    <div
      className={`bg-white flex flex-col w-[320px] h-[100%] rounded-lg p-[30px] gap-6 ${className}`}
    >
      <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col items-start">
          <p className="text-[#414447] text-h2">{user?.name}</p>
          <p className="text-[#909091] text-[14px] font-medium leading-[24px] tracking-[-0.2px]">
            {goalData.name}
          </p>
        </div>
      </div>
      ...
      <div className="self-stretch flex-1 overflow-hidden flex flex-col">
        ...
        <div
          ...
          className={`space-y-4 overflow-y-auto ${
            showScrollbar ? "minimal-scrollbar" : "minimal-scrollbar"
          }`}
          ...
          {projectsLoading ? (
            ...
            skillUsage.map((skill) => (
              <div key={skill.name}>
                ...
                <div className="w-[98%] bg-gray-200 rounded-full h-2.5 dark:bg-gray-700">
                  <div
                    className="bg-[#1FD167] h-2.5 rounded-full"
