...
import {
  useGetJobLocationSuggestionsMutation,
  useGetJobRoleSuggestionsMutation,
...
const JobsFilterModal: React.FC<JobsFilterModalProps> = (props) => {
  ...
  const [
    getJobRoleSuggestion,
    {
      data: fetchedSuggestedJobRoles,
  ...
  const [
    ...
    { data: fetchedSuggestedLocations, isLoading, error },
  ] = useGetJobLocationSuggestionsMutation();
  ...
  const [mininmumExperience, setMinimumExperience] = useState(
  ...
  useEffect(() => {
    if (fetchedSuggestedLocations != undefined) {
      const newLocations = fetchedSuggestedLocations.locations.filter(
        (item) => {
          return !selectedLocations.includes(item);
        }
      );
  ...
  useEffect(() => {
    if (fetchedSuggestedJobRoles != undefined) {
      const newJobTitles = fetchedSuggestedJobRoles.roles.filter((item) => {
        return !selectedJobTitles.includes(item);
      });
  ...
  const addLocationToFilters = (location: string) => {
    1;
  ...
  return (
    <div
      className="p-[42px] z-[52] flex items-center justify-center  w-full h-full absolute top-0 left-0  bg-black/50"
      onClick={() => {
        setIsFilterModalOpen(false);
      }}
    >
      <div
        ...
        className="flex flex-col gap-8  m-auto w-[50%] h-full bg-white p-[42px] rounded-[8px] overflow-auto scrollbar-hide"
      ...
      >
        ...
        <section className="flex flex-row justify-between">
          ...
          <img
            src={closesvg}
            className="text-gray-600 bg-white border-none p-2.5"
            onClick={() => {
              setIsFilterModalOpen(false);
            }}
          ></img>
              ...
              <div className="h-min-16 p-4  bg-white rounded-md border border-black/10 justify-start items-center flex flex-wrap gap-4 ">
                <div className="">
                  <ChipsCard
                    ...
                    itemList={selectedJobTitles}
                    setItemList={(items: string[]) =>
                      setSelectedJobTitles(items)
                    }
                  ></ChipsCard>
                </div>
                <div className="flex-grow items-end inline-block">
                  <SearchPopover
                    onInputChange={handleJobTitleSearch}
                    options={suggestedJobTitles}
                    onSelect={addJobTitletoFilters}
                  ></SearchPopover>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className=" justify-start items-center flex flex-row gap-4 ">
                  <ChipsCardAdd
                    ...
                    selectItem={addJobTitletoFilters}
                  ></ChipsCardAdd>
                </div>
              </div>
            </div>
              ...
              <div className="relative flex  gap-6 w-1/2">
                ...
                <div className="relative w-full ">
                  ...
                  <input
                    ...
                    max="31"
                    value={
                      mininmumExperience !== null ? mininmumExperience : 31
                    }
                    onChange={(e: any) =>
                      e.target.value == 31
              ...
              <div className="flex flex-col gap-3 ">
                ...
                <div className="justify-start items-center flex flex-row gap-4 ">
                  <ChipsCardAdd
                    ...
                    selectItem={addLocationToFilters}
                  ></ChipsCardAdd>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3 ">
                ...
                <div className="h-[50px] px-4 py-2 bg-white rounded-md border border-black/10 justify-start items-center gap-3.5 inline-flex">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      setMinimumSalary(e.target.valueAsNumber);
                    }}
                    value={minimumSalary}
                  />
                </div>
          ...
          <div className="flex flex-row gap-5 w-full">
            ...
            <div className="flex flex-col gap-3.5  w-full">
              ...
              <div className="px-6 pt-8 pb-6 bg-[#f9f9f9] rounded-[7px] border border-black/10 flex-col justify-start items-start gap-6 inline-flex  h-full">
                <CheckBox
                  itemList={jobTypeMapping}
                  multiselect={true}
                  selected={jobTypes}
                  onSelected={handleJobTypeSelection}
                ></CheckBox>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3.5 w-full">
              ...
              <div className="px-6 pt-8 pb-6 bg-[#f9f9f9] rounded-[7px] border border-black/10 flex-col justify-start items-start gap-6 inline-flex ">
                <CheckBox
                  itemList={companySizes.map((item) => ({
                    value: item,
                    label: item + " emplyees",
                  }))}
                  multiselect={false}
                  selected={companySize}
                  onSelected={changeCompanysize}
                ></CheckBox>
              </div>
            </div>
          </div>
