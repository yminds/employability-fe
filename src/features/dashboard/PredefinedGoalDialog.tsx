...
const PredefinedGoalDialog: React.FC<GoalFormDialogProps> = ({
  isOpen,
  setIsOpen,
  selectedGoal,
  isSetGoalsPage,
  ...
  const [jobMarketDemand] = useState(
    selectedGoal
      ? jobsMarketDemandObj[
          Number(
            selectedGoal.job_market_demand
          ) as keyof typeof jobsMarketDemandObj
        ]
      : ""
  );
  const [minSalaryRange] = useState(
    selectedGoal ? selectedGoal.min_salary_range : 0
  );
  const [maxSalaryRange] = useState(
    selectedGoal ? selectedGoal.max_salary_range : 0
  );
  const [difficultyLevel] = useState(
    selectedGoal
      ? difficultyLevelObj[
          Number(
            selectedGoal.difficulty_level
          ) as keyof typeof difficultyLevelObj
        ]
      : ""
  );
  const [learningTime] = useState(
    selectedGoal
      ? learningTimeObj[
          Number(selectedGoal.learning_time) as keyof typeof learningTimeObj
        ]
      : ""
  );
  const [experienceLevel] = useState(
    selectedGoal
      ? experienceLevelObj[
          Number(
            selectedGoal.experience_level
          ) as keyof typeof experienceLevelObj
        ]
      : ""
  );
  ...
  const dialogClassName = `p-0 max-w-[1400px] h-[90vh]  overflow-y-auto minimal-scrollbar rounded-[12px] font-ubuntu [&>button:last-child]:hidden lg:max-w-full md:max-w-lg ${
    isSetGoalsPage
      ? "fixed w-[50vw] h-[100vh] transition-all duration-300 border-none rounded-none " +
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className={dialogClassName}>
          ...
          <DialogDescription className="hidden">
            Select a predefined goal to set your journey
          </DialogDescription>
          <div>
            {/* Header Section */}
            <div className="flex flex-col items-start justify-center gap-5 p-6 px-8 h-[245px] relative ">
              <button
                className="flex items-center gap-4 text-gray-600 text-base font-normal leading-6 tracking-[0.24px] font-sf-pro z-[9999]"
                onClick={handleCloseGoals}
              >
                <img src={BackImg} alt="" className="w-4 z-[9999]" /> Back to
                Goals
              </button>

              <div className="flex flex-col items-start gap-5 z-[9999]">
                <div className="flex-col items-end gap-3 self-stretch inline-block">
                  <span className="text-[#414447] text-[32px] font-medium leading-[42px] tracking-[-0.5px]">
                    {goal}
                  </span>
                ...
                <button
                  className="flex w-[196px] h-[44px] p-4 px-8 justify-center items-center gap-2 rounded bg-[#00183D] text-white text-[16px] font-medium leading-[150%] font-sf-pro"
            ...
            <div className="p-6 px-8 font-sf-pro">
              ...
              <div className="flex pb-5">
                {tabs.map((tab) => (
                  <button
                    ...
                    className={`flex h-[36px] justify-center items-center gap-[10px] flex-[1_0_0] text-center text-base font-normal leading-6 tracking-wide ${
                      activeTab === tab
              ...
              {/* Market Trend Tabs */}
              {activeTab === "Market Trend" && <PredefinedGoalMarketTrend />}

              {/* Active Jobs Tabs */}
              {activeTab === "Active Jobs" && <PredefinedGoalActiveJobs />}
            </div>
          </div>
        </DialogContent>
      </Dialog>
