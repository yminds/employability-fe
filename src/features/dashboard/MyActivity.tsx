...
const CompleteActivityCard: React.FC<CompleteActivityProps> = ({
  displayScore = true,
  goalId,
  goalName,
}) => {
    ...
    >
      ...
      <div className="flex items-center gap-3 relative z-10">
        <Avatar className="h-[42px] w-[42px]">
          {user?.profile_image ? (
            <AvatarImage
              src={user?.profile_image || "/placeholder.svg?height=90&width=90"}
              alt={user?.name}
            />
          ) : (
            <AvatarImage
        ...
        <div className="space-y-0.5">
          <h2 className="text-h2 text-[#414447]">
            {user?.name || "User Name"}
          </h2>
      ...
      <button
        ...
        className="flex items-center gap-1.5 text-[#414447] hover:text-[#000000] transition-colors group relative z-10"
      >
