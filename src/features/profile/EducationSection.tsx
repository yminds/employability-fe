...
const EducationSection: React.FC<EducationSectionProps> = ({ }) => {
  ...
  const openModal = (educationItem?: Education) => {
    setEditingEducation(educationItem || null);
    setIsModalOpen(true);
  };
  ...
  return (
    <div className="rounded-lg bg-white overflow-hidden">
      ...
      {/* Scrollable Content */}
      <div className="overflow-y-auto max-h-[600px] px-6 py-4 " style={{ scrollbarWidth: "none" }}>
        {education.length === 0 ? (
          <p className="text-gray-500">No education added yet.</p>
        ) : (
          <div className="space-y-6 ">
            {education.map((entry, index) => (
              <div key={index} className="flex items-start gap-4 border-b border-gray-200 px-2 pb-4 ">
                <div className="flex-shrink-0 mt-1">
                  {entry.highest_education_level?.toLowerCase()?.includes("secondary") ? (
                ...
                <div className="flex-1">
                  <h3 className="font-medium text-gray-900">{entry.degree || "N/A"}</h3>
                  <p className="text-sm text-gray-500">{entry.institute || "N/A"}</p>
                  <p className="text-sm text-gray-500">{entry.board_or_certification || "N/A"}</p>
                </div>
                <div className="text-right">
                  ...
                  <p className="text-sm text-gray-500">
                    {entry.from_date ? new Date(entry.from_date).toLocaleDateString() : "N/A"} -
                    {entry.till_date ? new Date(entry.till_date).toLocaleDateString() : "N/A"}
                  </p>
                </div>
              </div>
            ))}
